...
import frontend.objects.menu.Profile.User;
...
class Common {
    ...
    public static function switchState(targetSectionOrState:OneOfTwo<String, FlxState>, ?arg:Dynamic = null, ?transIn:Bool = true)
    {
        if (Std.isOfType(targetSectionOrState, String)){
            ...
            if (Std.isOfType(targetSectionOrState, frontend.states.MenuState)){
                FlxG.switchState(new frontend.states.MenuState(cast arg));
                return;
            }
