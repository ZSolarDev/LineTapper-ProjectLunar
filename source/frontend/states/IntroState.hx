    ...
    function postLoadGame()
    {
        ...
        Thread.create(() -> {
            while(!doneLoading){
                ...
                {
                    trace('\n|| Error while loading: ${e.message}\nDetails: ${e.stack}\n|| Trying again!');
                    try{
                        curLoadingStage = 0;
                        loadGame();
                    }catch(e)
                    {
                        trace('\n|| Error while loading: ${e.message} | Details: ${e.stack}\n|| Please report this to ZSolarDev, loading skipped.');
                        curLoadingStage = loadingStages;
                        doneLoading = true;
                    }
                }
            }
